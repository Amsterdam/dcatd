...
class DatasetTestCase(BaseTestCase):
    ...
    async def test_dataset_operations(self):
        ...
        with open(
                self._WORKING_PATH + path.sep + 'test_update.json') as definition:
            updated_data = definition.read()
        ...
        self.assertEquals(response.status, 304,
                          'Geen geldige response voor If-none-match request ontvangen')

        response = await self.client.request(
            "GET", f"/datasets/{docid}", headers={'If-None-Match': 'notanetag'})
