...
node {
    ...
    stage("Build image") {
        tryStep "build", {
            def image = docker.build("repo.data.amsterdam.nl/datapunt/dcatd:${env.BUILD_NUMBER}")
            image.push()
        }
    }
}
    ...
    node {
        stage('Push acceptance image') {
            tryStep "image tagging", {
                def image = docker.image("repo.data.amsterdam.nl/datapunt/dcatd:${env.BUILD_NUMBER}")
                image.pull()
